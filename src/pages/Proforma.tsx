...
const STATUS_CFG: Record<string, { label: string; cls: string }> = {
  ...
  ACCEPTEE:  { label: 'Signée',    cls: 'bg-green-100 text-green-700' },
  PAYEE:     { label: 'Facturée',  cls: 'bg-emerald-100 text-emerald-700' },
  ANNULEE:   { label: 'Refusée',   cls: 'bg-red-100 text-red-700' },
}
...
export default function Proforma() {
  ...
  const all      = ((data as any[]) ?? []).filter(i => i.type === 'PROFORMA')
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if ((!clientId && !prospectId) || !dueDate || lines.some(l => !l.description || l.unitPrice <= 0)) {
      setFormError('Prospect/client, date et toutes les lignes sont obligatoires.')
      return
    }
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div className="bg-white rounded-xl p-4 border border-green-200 shadow-sm">
          ...
          <p className="text-xs text-slate-500">Proforma signées</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-4 border-b border-slate-100">
          ...
          <button onClick={() => { resetModal(); setShowModal(true) }}
            ...
            <Plus size={13} /> Nouvelle proforma
          </button>
        </div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {inv.status === 'BROUILLON' && (
                            <button onClick={async () => { await updateInvoiceStatus(inv.id, 'ENVOYEE'); reload() }}
                              className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-500 hover:text-blue-700" title="Marquer comme envoyée">
                              <Send size={14} />
                            </button>
                          )}
                          {inv.status === 'ENVOYEE' && (
                            <>
                              <button onClick={async () => { await updateInvoiceStatus(inv.id, 'ACCEPTEE'); reload() }}
                                className="p-1.5 rounded-lg hover:bg-green-50 text-green-500 hover:text-green-700" title="Signée / Acceptée">
                                <CheckCircle size={14} />
                              </button>
                              <button onClick={async () => { await updateInvoiceStatus(inv.id, 'ANNULEE'); reload() }}
                                className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 hover:text-red-600" title="Refusée">
                                <XCircle size={14} />
                              </button>
                            </>
                          )}
              ...
              <div className="px-4 sm:px-6 py-5 space-y-4">
                ...
                <div className="bg-purple-50 border border-purple-200 rounded-xl p-3">
                  <p className="text-xs text-purple-700">
                    <b>💡 La facture proforma</b> est envoyée au prospect pour signature. Une fois signée, le prospect est converti en client et le contrat + facture définitive sont créés.
                  </p>
                </div>
