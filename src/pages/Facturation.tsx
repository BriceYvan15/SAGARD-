                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1.5">
                          {inv.status !== 'PAYEE' && inv.status !== 'ANNULEE' && inv.type === 'FACTURE' && (
                            <button onClick={() => { setPayModalInvoice(inv); setPayMethod('CHEQUE') }}
    ...
    {payModalInvoice && (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h2 className="text-base font-bold text-slate-800">Confirmer le paiement</h2>
            <button onClick={() => setPayModalInvoice(null)} className="p-1.5 rounded-lg hover:bg-slate-100"><X size={18} /></button>
          </div>
          <div className="px-6 py-5 space-y-4">
            <div className="bg-slate-50 rounded-lg p-3">
              <p className="text-xs text-slate-500">Facture</p>
              <p className="text-sm font-bold text-slate-800">{payModalInvoice.invoiceNumber}</p>
              <p className="text-sm text-slate-600 mt-1">Montant : <span className="font-bold">{fmt(Number(payModalInvoice.totalAmount ?? 0))} FCFA</span></p>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Mode de paiement</label>
              <select value={payMethod} onChange={e => setPayMethod(e.target.value)}
                className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-sagard-yellow/40">
                {PAYMENT_METHODS.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
              </select>
            </div>
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setPayModalInvoice(null)} className="px-4 py-2 rounded-lg border border-slate-200 text-sm">Annuler</button>
              <button disabled={payingLoading} onClick={async () => {
                ...
                {payingLoading ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle size={14} />}
                Confirmer
              </button>
            </div>
          </div>
